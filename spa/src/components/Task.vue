<template>
    <div class="content" :order="task.order" :id="task.id"> 
      <div class="card level columns is-mobile is-gapless" ref="taskContainer">
        ...
        <!-- edit task button-->
              <a v-if="!editing && !deleting && !task.addingNew" @click="edit">
                  <span class="icon is-medium"><i class="icon-pencil"></i></span>
              </a>
        <!-- save task button -->
              <a v-if="editing && !deleting && !task.addingNew" @click="saveTask">
                  <span class="icon is-medium" style="height: 4em;"><i class="icon-checkmark"></i></span>
              </a>
        <!-- delete task button -->
              <a v-if="!deleting && !editing && !task.addingNew" @click="clickDelete">
                  <span class="icon is-medium"><i class="icon-bin"></i></span>
              </a>
        <!-- confirm delete task button -->
              <a v-if="deleting && !task.addingNew" @click="deleteTask">
                  <span class="icon is-medium"><i class="icon-checkmark"></i></span>
              </a>
        <!-- cancel delete button -->
              <a v-if="deleting && !task.addingNew" @click="deleting = false">
                  <span class="icon is-medium"><i class="icon-cross"></i></span>
              </a>
              
        <!-- buttons for new task -->
              <a v-if="task.addingNew" @click="saveNewTask">
                  <span class="icon is-medium"><i class="icon-checkmark"></i></span>
              </a>
              <a v-if="task.addingNew" @click="discardNewTask">
                  <span class="icon is-medium"><i class="icon-cross"></i></span>
              </a>
